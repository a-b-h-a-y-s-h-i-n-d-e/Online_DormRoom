...
def login_page(client):
    ...
    username_label = Label(frame,text='Enter Username :',font=('Roboto',20),bg="black",fg='#149414',width=15,height=1,)
    username_label.place(x=225, y=150)
    entry_text = StringVar()
    username_entry = Entry(frame, font=('Roboto',15), bg='#FFFFFF', fg='#5F8575', width=18, textvariable=entry_text,justify=CENTER)
    username_entry.place(x=225, y=200)
    username_entry.bind('<Return>',lambda event: save_username())
    
    login_btn = Button(frame, text='LOGIN', height=1,width=13,command=save_username,  font=('Roboto',14), bg='#149414', fg='#000000')
...
def chat_page(client, USERNAME):
    ...
    def you_joins():
        ...
        msg_box.tag_configure("center", justify="center", foreground="black", background="orange", font=("Roboto", 15))
        msg_box.insert(END, '\n')
    ...
    def display_msg(username, msg):
        ...
        msg_box.configure(state=NORMAL)
        msg_box.tag_configure('left', justify='left', foreground='#581515', background='white',  font=('Roboto', 17), rmargin=15)
    ...
    def send_message(event = None):
        ...
        if msg != '':
            ...
            dis_msg = f'You : {msg}'
            msg_box.configure(state=NORMAL)
            msg_box.tag_configure('right', justify='right', foreground='white', font=('Roboto', 17), rmargin=15)
            msg_box.insert(END, dis_msg + '\n', 'right')
            msg_box.insert(END, '\n')
            msg_box.configure(state=DISABLED)
    ...
    msg_entry = Entry(window,bg="#FFFFFF",font=('Roboto',20),width=25)
    ...
    on_box = Text(window,bg = "#8A9A5B",width = 13, height = 14, state = NORMAL, font=('Roboto',17), fg='#F0FFFF')
    ...
    quit_button =  Button(window,text='EXIT', height=1,width=14,  font=('Roboto',17),
